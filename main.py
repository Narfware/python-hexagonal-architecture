...
def main():
    ...
    mongo_order_repository = Mongo_Order_Repository(
        mongo_client
    )  ## Inject mongodb in the implementation

    create_order_use_case = Create_Order(
        mongo_order_repository, Nano_id()
    )  ## Inject manually the dependencies in the use_case

    # Primary adapters
    return FastApiManager(
        create_order_use_case
    ).initialize()  ## Inject manually the useCases in the primary adapters
