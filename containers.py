...
class Container(containers.DeclarativeContainer):
    ...
    order_repository = providers.Singleton(
        Mongo_Order_Repository, mongo_client
    )  # Inject the database client connected

    ## ID GENERATOR
    id_generator = providers.Singleton(
        Nano_id
    )  # Inject the database identifier generator

    create_order_use_case = (
        providers.Factory(  # Inject the use case with the necessary dependencies
            Create_Order,
